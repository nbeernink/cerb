            ...
            try {
                ...
                if ($mail_transport->id == CerbMailTransport_Smtp::ID) {
                    ...
                    $fields = [
                        ...
                        DAO_MailTransport::IS_DEFAULT   => 1,
                        DAO_MailTransport::PARAMS_JSON  => json_encode([]),
                    ];
                    $transport_id = DAO_MailTransport::create($fields);
                }
                ...
                if ($transport_id) {
                    ...
                    if (false != ($default_replyto = DAO_AddressOutgoing::getDefault())) {
                        DAO_AddressOutgoing::update($default_replyto->address_id, [
    ...
    case STEP_DEFAULTS:
        ...
        if (!empty($form_submit)) {
            ...
            if (!empty($worker_email) && !empty($worker_pass) && $worker_pass == $worker_pass2) {
                ...
                if (empty($groups)) {
                    // Dispatch Group
                    $dispatch_gid = DAO_Group::create([
                        DAO_Group::NAME => 'Dispatch',
                    ]);

                    // Support Group
                    $support_gid = DAO_Group::create([
                        DAO_Group::NAME => 'Support',
                    ]);

                    // Sales Group
                    $sales_gid = DAO_Group::create([
                        DAO_Group::NAME => 'Sales',
                    ]);

                    // Default catchall
                    DAO_Group::update($dispatch_gid, [
                ...
                if (empty($roles)) {
                    $fields = [
                        ...
                        DAO_WorkerRole::PARAMS_JSON => json_encode([
                            'who'  => 'all',
                            'what' => 'all',
                        ]),
                    ];
                ...
                if (null == ($lookup = DAO_Worker::getByEmail($worker_email))) {
                    ...
                    $fields = [
                        ...
                        DAO_Calendar::PARAMS_JSON      => json_encode([
                            'manual_disabled' => '0',
                            'sync_enabled'    => '0',
                            'start_on_mon'    => '1',
                            'hide_start_time' => '0',
