...
class DevblocksTourCallout
{
    ...
    public function __construct($selector='', $title='Callout', $body='...', $tipCorner='topLeft', $targetCorner='topLeft', $xOffset=0, $yOffset=0)
    {
...
class DevblocksSearchFieldContextKeys
{
    ...
    public function __construct($where_key, $field_key=null, $dict_key=null)
    {
...
{
    public static function getCustomFieldContextData($context)
    {
    ...
    public static function getCustomFieldContextWhereKey($context)
    {
        $where_key = null;
    ...
    public static function getCustomFieldContextFieldKey($context)
    {
        $field_key = null;
    ...
    public static function _getWhereSQLFromFulltextField(DevblocksSearchCriteria $param, $schema, $pkey)
    {
    ...
    public static function _getWhereSQLFromCommentFulltextField(DevblocksSearchCriteria $param, $schema, $from_context, $pkey)
    {
    ...
    public static function _getWhereSQLFromWatchersField(DevblocksSearchCriteria $param, $from_context, $pkey)
    {
    ...
    public static function _getWhereSQLFromCustomFields($param)
    {
...
class DevblocksSearchCriteria
{
    ...
    public static function getWorkerParamFromTokens($field_key, $tokens, $search_field)
    {
        ...
        } elseif (1 == count($terms) && in_array(strtolower($terms[0]), array('blank','empty','no','none','noone','nobody'))) {
            @$is_cfield = $search_field['options']['cf_id'];
            if ($is_cfield) {
    ...
    public static function getWatcherParamFromTokens($field_key, $tokens)
    {
        ...
        if (1 == count($terms) && in_array(strtolower($terms[0]), array('any','yes'))) {
            $oper = self::OPER_IS_NOT_NULL;
            $value = array();
        } elseif (1 == count($terms) && in_array(strtolower($terms[0]), array('none','no'))) {
            $oper = self::OPER_IS_NULL;
            $value = array();
        } else {
            $active_worker = CerberusApplication::getActiveWorker();
    ...
    protected static function _escapeSearchParam(DevblocksSearchCriteria $param, $fields)
    {
        $db = DevblocksPlatform::getDatabaseService();
...
class DevblocksSearchField
{
    ...
    public function __construct($token, $db_table, $db_column, $label=null, $type=null, $is_sortable=false)
    {
        $this->token = $token;
    ...
    public static function getCustomSearchFieldsByContexts($contexts)
    {
...
class DevblocksPluginManifest
{
    ...
    public $link = '';
    public $dir = '';
    public $manifest_cache = array();
    
    public $extension_points = array();
    public $event_points = array();
    public $acl_privs = array();
    public $class_loader = array();
    public $extensions = array();
    
    public $_requirements_errors = array();
    ...
    public function checkRequirements()
    {
        ...
        if (isset($this->manifest_cache['dependencies'])) {
            ...
            foreach ($this->manifest_cache['dependencies'] as $dependency) {
                ...
                } elseif (!$plugins[$dependency]->enabled) {
    ...
    public function _recursiveDelTree($dir)
    {
...
class DevblocksExtensionManifest
{
    ...
    public function DevblocksExtensionManifest()
    {
    }
