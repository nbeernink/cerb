...
DevblocksPlatform::registerClasses($path.'Mail.php', [
    'CerberusMail',
]);

DevblocksPlatform::registerClasses($path.'Parser.php', [
    'CerberusParser',
...
DevblocksPlatform::registerClasses($path.'Update.php', [
    'ChUpdateController',
]);

DevblocksPlatform::registerClasses($path.'Utils.php', [
    'CerberusUtils',
]);
...
class CerberusApplication extends DevblocksApplication
{
    ...
    public static function setActiveWorker($worker)
    {
     ...
     * @return Model_Worker
     */
    public static function getActiveWorker()
    {
    ...
    public static function getWorkersByAtMentionsText($text)
    {
        $workers = [];
    ...
    public static function getWorkerPickerData($population, $sample, $group_id = 0, $bucket_id = 0)
    {
        // Shared objects
    ...
    public static function getFileBundleDictionaryJson()
    {
        $file_bundles = DAO_FileBundle::getAll();
        $active_worker = self::getActiveWorker();
    ...
    public static function getAtMentionsWorkerDictionaryJson()
    {
     ...
     */
    public static function getPageManifestByUri($uri)
    {
    ...
    public static function processRequest(DevblocksHttpRequest $request, $is_ajax = false)
    {
        /*
    ...
    public static function checkRequirements()
    {
        ...
        if (!is_writable(APP_SMARTY_COMPILE_PATH)) {
     ...
     */
    public static function generateTicketMask($pattern = null)
    {
        if (empty($pattern)) {
    ...
    public static function generateTicketMaskCardinality($pattern = null)
    {
        if (empty($pattern)) {
            $pattern = DevblocksPlatform::getPluginSetting('cerberusweb.core', CerberusSettings::TICKET_MASK_FORMAT);
        }
     ...
     */
    public static function hashLookupAddress($email_or_id, $create = false)
    {
    ...
    public static function hashLookupAddresses($emails_or_ids, $create = false)
    {
        $results = [];
     ...
     * @todo [JAS]: Move this to a global cache/hash registry
     */
    public static function hashLookupOrg($name_or_id, $create = false)
    {
        static $hash_to_org = [];
        static $hash_hits = [];
        static $hash_size = 0;
    ...
    public static function hashLookupOrgs($names_or_ids, $create = false)
    {
        $results = [];
     ...
     *
     * @return int The ticket id, or NULL if not found
     *
     * @todo [JAS]: Move this to a global cache/hash registry
     */
     ...
     * @return array
     */
    public static function saveHttpUploadedFiles($files)
    {
        $file_ids = [];
...
{
    public static function getValue($context, $context_values);
}
...
class CerberusContexts
{
    ...
    public static function getContext($context, $context_object, &$labels, &$values, $prefix = null, $nested = false, $skip_labels = false)
    {
            ...
            default:
                ...
                if (null != ($ctx = Extension_DevblocksContext::get($context))) {
                    ...
                    if (is_null($context_object)) {
                        ...
                        $hash = md5(serialize([$context, $prefix, $nested]));
                        ...
                        // Cache miss
                            ...
                            $cache->save(['labels' => $loaded_labels, 'values' => $loaded_values], $cache_key, [], 0, true);
                        }
                        ...
                        if (self::$_is_caching_loads) {
                            ...
                            $hash = md5(serialize([$context, $hash_context_id, $prefix, $nested]));
        ...
        if (!$nested) {
            ...
            if ($context != self::CONTEXT_WORKER) {
    ...
    public static function merge($token_prefix, $label_prefix, $src_labels, $src_values, &$dst_labels, &$dst_values)
    {
        ...
        if (is_array($src_values)) {
            foreach ($src_values as $token => $value) {
                if (in_array($token, ['_labels', '_types'])) {
                    switch ($token) {
                    case '_labels':
    ...
    {
        if (false == ($a = self::polymorphActor($a))) {
            return false;
        }

        if (false == ($b = self::polymorphActor($b))) {
            return false;
        }
        ...
        if (is_array($actor)) {
            ...
            switch ($actor_context) {
                ...
                case self::CONTEXT_GROUP:
                    ...
                    break;
                case self::CONTEXT_WORKER:
                    $actor = DAO_Worker::get($actor_context_id);
                    break;
                case self::CONTEXT_VIRTUAL_ATTENDANT:
    ...
    {
        if (false == ($actor = self::polymorphActor($actor))) {
            return false;
        }
            ...
            case self::CONTEXT_ROLE:
                switch (get_class($actor)) {
                    ...
                    case 'Model_VirtualAttendant': /* @var $actor Model_VirtualAttendant */
                        return self::isReadableByActor($owner_context, $owner_context_id, [$actor->owner_context, $actor->owner_context_id]);
                        break;
                }
                break;
        ...
        switch ($owner_context) {
            case self::CONTEXT_APPLICATION:
                switch (get_class($actor)) {
                    ...
                    case 'Model_VirtualAttendant': /* @var $actor Model_VirtualAttendant */
                        return self::isWriteableByActor($owner_context, $owner_context_id, [$actor->owner_context, $actor->owner_context_id]);
                        break;
                }
                break;
    ...
    {
        $links = DAO_ContextLink::getContextLinks($context, $context_id, self::CONTEXT_WORKER);
        ...
        if ($as_contexts) {
            if (is_array($watcher_ids)) {
                foreach ($watcher_ids as $watcher_id) {
                    ...
                    self::getContext(self::CONTEXT_WORKER, $watcher_id, $null_labels, $watcher_values, null, true);
    ...
    public static function addWatchers($context, $context_id, $worker_ids)
    {
        ...
        foreach ($worker_ids as $worker_id) {
            if (null != ($worker = @$workers[$worker_id]) && $worker instanceof Model_Worker && !$worker->is_disabled) {
                DAO_ContextLink::setLink($context, $context_id, self::CONTEXT_WORKER, $worker_id);
            }
        }
    }
    ...
    public static function removeWatchers($context, $context_id, $worker_ids)
    {
        ...
        foreach ($worker_ids as $worker_id) {
            DAO_ContextLink::deleteLink($context, $context_id, self::CONTEXT_WORKER, $worker_id);
        }
    }

    public static function formatActivityLogEntry($entry, $format = null, $scrub_tokens = [], $personalize = false)
    {
    ...
    public static function parseContextUrl($url)
    {
        if (0 != strcasecmp('ctx://', substr($url, 0, 6))) {
            return false;
        }
    ...
    public static function pushActivityDefaultActor($context = null, $context_id = null)
    {
        if (empty($context) || empty($context_id)) {
            ...
            self::$_default_actor_stack[] = [$context, $context_id];
        }
    }
    ...
    public static function getCurrentActor($actor_context = null, $actor_context_id = null)
    {
        ...
        if (empty($actor_context)) {
            ...
            if (EventListener_Triggers::getDepth() > 0
                ...
                $actor_context = self::CONTEXT_VIRTUAL_ATTENDANT;
                $actor_context_id = $trigger_va->id;
                $actor_url = sprintf('ctx://%s:%d', self::CONTEXT_VIRTUAL_ATTENDANT, $trigger_va->id);
            ...
            // Otherwise see if we have an active session
                ...
                if (empty($actor_context) && null != ($active_worker = CerberusApplication::getActiveWorker())) {
                    ...
                    $actor_context = self::CONTEXT_WORKER;
        ...
        if (empty($actor_context)) {
            $actor_context = self::CONTEXT_APPLICATION;
            $actor_context_id = 0;
            $actor_name = 'Cerb';
            $actor_url = null;
        }
    ...
    public static function logActivity($activity_point, $target_context, $target_context_id, &$entry_array, $actor_context = null, $actor_context_id = null, $also_notify_worker_ids = [], $also_notify_ignore_self = false)
    {
        ...
        if ($do_notifications) {
            ...
            if (self::CONTEXT_WORKER != $actor['context']) {
                $watchers = array_merge(
                    $watchers,
                    array_keys(self::getWatchers($actor['context'], $actor['context_id']))
                );
            }

            // And watchers of the target (if not a worker)
            if (self::CONTEXT_WORKER != $target_context) {
                $watchers = array_merge(
                    $watchers,
                    array_keys(self::getWatchers($target_context, $target_context_id))
                );
            }
            ...
            if (is_array($target_custom_fields)) {
                foreach ($target_custom_fields as $target_custom_field_id => $target_custom_field) {
                    ...
                    if (isset($values[$target_context_id]) && isset($values[$target_context_id][$target_custom_field_id])) {
                        $watchers = array_merge(
                        $watchers,
                        [$values[$target_context_id][$target_custom_field_id]]
                    );
                    }
                }
            }
            ...
            if (is_array($watcher_ids)) {
                ...
                foreach ($watcher_ids as $watcher_id) {
                    ...
                    if (0 == EventListener_Triggers::getDepth()) {
                        ...
                        if ($actor['context'] == self::CONTEXT_WORKER
    ...
    public static function getModels($context, array $ids)
    {
    ...
    private static $_context_checkpoints = [];

    public static function checkpointChanges($context, $ids)
    {
        ...
        if (!empty($load_ids)) {
            $models = self::getModels($context, $load_ids);
    ...
    public static function getCheckpoints($context, $ids)
    {
        $models = [];
    ...
    public static function shutdown()
    {
        ...
        foreach (self::$_context_checkpoints as $context => &$old_models) {
            ...
            foreach (array_chunk($ids, 100) as $context_ids) {
                $new_models = self::getModels($context, $context_ids);
...
{
    public function authorize($context_id, Model_Worker $worker)
    {
        // Security
        try {
            if (empty($worker)) {
    ...
    public function getContext($object, &$token_labels, &$token_values, $prefix = null)
    {
        if (is_null($prefix)) {
            $prefix = 'Application:';
        }
    ...
    public function getChooserView($view_id = null)
    {
    }

    public function getView($context = null, $context_id = null, $options = [], $view_id = null)
    {
    }
}
...
class CerberusLicense
{
    ...
    {
        try {
            $l = (null != (@$data = json_decode(DevblocksPlatform::getPluginSetting('cerberusweb.core', CerberusSettings::LICENSE, ''), true))) ? $data : [];
            self::validate(base64_decode(@$l[base64_decode('ZGF0YQ==')]), @$l['company'], @$l['email']) ? @$this->data = $l : @$this->data = [];
        } catch (Exception $e) {
            @$this->data = [];
        }
    ...
    {
        switch ($property) {case 'w':return max(1, @intval($this->seats));
        break;
        default:return(isset($this->data[$property])) ? $this->data[$property] : null;
        break;
    }
    }
    ...
    {
        try {
            foreach (['L0tleTogKC4qKS8=' => 's', 'L0NyZWF0ZWQ6ICguKikv' => 'c', 'L1VwZGF0ZWQ6ICguKikv' => 'u', 'L1VwZ3JhZGVzOiAoLiopLw==' => 'e', 'L1NlYXRzOiAoLiopLw==' => 'w'] as $k => $v) {
                @preg_match(base64_decode($k), $key, $matches) ? @$$v = trim($matches[1]) : null;
            }
            $r = [];
            @$w = intval($w);
            @$cp = base64_decode('Y29tcGFueQ==');
            @$em = base64_decode('ZW1haWw=');
            @$cd = preg_replace('/[^A-Z0-9]/', '', $s);
            @$l = explode('-', $e);
            @$e = gmmktime(0, 0, 0, $l[1], $l[2], $l[0]);
            @$l = explode('-', $c);
            @$c = gmmktime(0, 0, 0, $l[1], $l[2], $l[0]);
            @$l = explode('-', $u);
            @$u = gmmktime(0, 0, 0, $l[1], $l[2], $l[0]);
            @$h = str_split(strtoupper(sha1(sha1('cerb5').sha1($$cp).sha1($$em).sha1(intval($w)).sha1(gmdate('Y-m-d', $c)).sha1(gmdate('Y-m-d', $e)))), 1);
            if (0 == @strcasecmp(sprintf('%02X', strlen($$cp) + intval($w)), substr($cd, 3, 2)) && @intval(hexdec(substr($cd, 5, 1)) == @intval(bindec(sprintf('%d%d%d%d', (182 <= gmdate('z', $e)) ? 1 : 0, (5 == gmdate('w', $e)) ? 1 : 0, ('th' == gmdate('S', $e)) ? 1 : 0, (1 == gmdate('w', $e)) ? 1 : 0)))) && 0 == @strcasecmp($h[hexdec(substr($cd, 1, 2)) - @hexdec(substr($cd, 0, 1))], substr($cd, 0, 1))) {
                @$r = [base64_decode('a2V5') => $s, base64_decode('Y3JlYXRlZA==') => $c, base64_decode('dXBkYXRlZA==') => $u, base64_decode('dXBncmFkZXM=') => $e, @$cp => $$cp, @$em => $$em, base64_decode('c2VhdHM=') => intval($w), base64_decode('ZGF0YQ==') => base64_encode($key)];
            }
...
class Cerb_DevblocksSessionHandler implements IDevblocksHandler_Session
{
    ...
    public static function destroyByWorkerIds($ids)
    {
        if (!self::isReady()) {
            return false;
        }
...
{
    public static $_worker = null;
    public static $_plugin_cache = [];

    public static function shouldLoadExtension(DevblocksExtensionManifest $extension_manifest)
    {
        // Always allow core
...
class Cerb_ORMHelper extends DevblocksORMHelper
{
    ...
    public static function recastArrayToModel($array, $model_class)
    {
    ...
    public static function uniqueFields($fields, $model)
    {
        if (is_object($model)) {
            $model = (array) $model;
        }
    ...
    protected static function paramExistsInSet($key, $params)
    {
        $exists = false;
    ...
    protected static function _getRandom($table, $pkey = 'id')
    {
        $db = DevblocksPlatform::getDatabaseService();
    ...
    public static function _searchComponentsVirtualOwner(&$param, &$join_sql, &$where_sql)
    {
        $worker_ids = DevblocksPlatform::sanitizeArray($param->value, 'integer', ['nonzero', 'unique']);
    ...
    public static function _searchComponentsVirtualHasFieldset(&$param, $to_context, $to_index, &$join_sql, &$where_sql)
    {
    ...
    public static function _searchComponentsVirtualContextLinks(&$param, $to_context, $to_index, &$join_sql, &$where_sql)
    {
        if ($param->operator != DevblocksSearchCriteria::OPER_TRUE) {
            if (empty($param->value) || !is_array($param->value)) {
                $param->operator = DevblocksSearchCriteria::OPER_IS_NULL;
