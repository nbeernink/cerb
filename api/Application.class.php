...
class CerberusContexts
{
    ...
    public static function formatActivityLogEntry($entry, $format=null, $scrub_tokens=array(), $personalize=false)
    {
    ...
    public static function logActivity($activity_point, $target_context, $target_context_id, &$entry_array, $actor_context=null, $actor_context_id=null, $also_notify_worker_ids=array(), $also_notify_ignore_self=false)
    {
        // Target meta
    ...
    private static $_context_checkpoints = array();
...
class Context_Application extends Extension_DevblocksContext
{
    ...
    public function getView($context=null, $context_id=null, $options=array(), $view_id=null)
    {
        return null;
    }
};
...
class CerberusLicense
{
    ...
    {
        try {
            $l=(null!=(@$data=json_decode(DevblocksPlatform::getPluginSetting('cerberusweb.core', CerberusSettings::LICENSE, ''), true)))?$data:array();
            self::validate(base64_decode(@$l[base64_decode('ZGF0YQ==')]), @$l['company'], @$l['email'])?@$this->data=$l:@$this->data=array();
        } catch (Exception $e) {
            @$this->data=array();
        }
        return;
    ...
    {
        try {
            foreach (array('L0tleTogKC4qKS8='=>'s','L0NyZWF0ZWQ6ICguKikv'=>'c','L1VwZGF0ZWQ6ICguKikv'=>'u','L1VwZ3JhZGVzOiAoLiopLw=='=>'e','L1NlYXRzOiAoLiopLw=='=>'w') as $k=>$v) {
            ...
            if (0==@strcasecmp(sprintf("%02X", strlen($$cp)+intval($w)), substr($cd, 3, 2))&&@intval(hexdec(substr($cd, 5, 1))==@intval(bindec(sprintf("%d%d%d%d", (182<=gmdate('z', $e))?1:0, (5==gmdate('w', $e))?1:0, ('th'==gmdate('S', $e))?1:0, (1==gmdate('w', $e))?1:0))))&&0==@strcasecmp($h[hexdec(substr($cd, 1, 2))-@hexdec(substr($cd, 0, 1))], substr($cd, 0, 1))) {
                @$r=array(base64_decode('a2V5')=>$s,base64_decode('Y3JlYXRlZA==')=>$c,base64_decode('dXBkYXRlZA==')=>$u,base64_decode('dXBncmFkZXM=')=>$e,@$cp=>$$cp,@$em=>$$em,base64_decode('c2VhdHM=')=>intval($w),base64_decode('ZGF0YQ==')=>base64_encode($key));
            }
            return $r;
        } catch (Exception $e) {
            return array();
...
class Cerb_DevblocksExtensionDelegate implements DevblocksExtensionDelegate
{
    ...
    public static $_plugin_cache = array();
