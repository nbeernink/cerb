        ...
        if (is_array($this->files)) {
            ...
            switch ($file->mime_type) {
                case 'message/rfc822':
                    ...
                    if (in_array(strtolower($bounce_froms[0]->mailbox), ['postmaster', 'mailer-daemon'])) {
...
class CerberusParserModel
{
    ...
    public function getRecipients()
    {
        ...
        if (isset($headers['to'])) {
            $sources = array_merge($sources, is_array($headers['to']) ? $headers['to'] : [$headers['to']]);
        }

        if (isset($headers['cc'])) {
            $sources = array_merge($sources, is_array($headers['cc']) ? $headers['cc'] : [$headers['cc']]);
        }

        if (isset($headers['envelope-to'])) {
            $sources = array_merge($sources, is_array($headers['envelope-to']) ? $headers['envelope-to'] : [$headers['envelope-to']]);
        }

        if (isset($headers['x-envelope-to'])) {
            $sources = array_merge($sources, is_array($headers['x-envelope-to']) ? $headers['x-envelope-to'] : [$headers['x-envelope-to']]);
        }

        if (isset($headers['delivered-to'])) {
            $sources = array_merge($sources, is_array($headers['delivered-to']) ? $headers['delivered-to'] : [$headers['delivered-to']]);
        }

        $destinations = [];
        foreach ($sources as $source) {
            @$parsed = imap_rfc822_parse_adrlist($source, 'localhost');
            $destinations = array_merge($destinations, is_array($parsed) ? $parsed : [$parsed]);
        }
...
class ParserFile
{
    ...
    public static function makeTempFilename()
    {
...
{
    public static function parseMessageSource($message_source, $delete_on_success = true, $delete_on_failure = true)
    {
        $file = null;
        ...
        try {
            if (preg_match('/^file:\/\/(.*?)$/', $message_source, $matches)) {
                ...
                if (null == ($parser_msg = self::parseMimeFile($file))) {
                    throw new Exception("The message mime could not be parsed (it's probably malformed).");
                }
            } else {
                $message_source .= PHP_EOL;

                if (null == ($parser_msg = self::parseMimeString($message_source))) {
                    throw new Exception("The message mime could not be parsed (it's probably malformed).");
                }
            }

            if (false === ($ticket_id = self::parseMessage($parser_msg))) {
    ...
    public static function parseMimeFile($full_filename)
    {
    ...
    public static function parseMimeString($string)
    {
    ...
    private static function _recurseMimeParts($part, &$results)
    {
        if (!is_array($results)) {
            $results = [];
        }
    ...
    private static function _getMimePartFilename($part)
    {
        ...
        return self::fixQuotePrintableString($content_filename, $part->data['charset']);
    }
     ...
     * @return CerberusParserMessage
     */
    private static function _parseMime($mm)
    {
        ...
        $message->headers = self::fixQuotePrintableArray($mm->data['headers']);
    ...
    private static function _handleMimepartTextPlain($section, CerberusParserMessage $message)
    {
    ...
    private static function _handleMimepartTextHtml($section, CerberusParserMessage $message)
    {
        @$transfer_encoding = $section->data['transfer-encoding'];
        $text = $section->extract_body(MAILPARSE_EXTRACT_RETURN);
     ...
     * @return int
     */
    public static function parseMessage(CerberusParserMessage $message, $options = [])
    {
        /*
         * options:
         * 'no_autoreply'
                ...
                if (!empty($proxy_ticket)) {
                    ...
                    if (is_array($lines)) {
                        ...
                        if (preg_match('/^#sig/', $line, $matches)) {
                            ...
                            if (!in_array($state, ['#cut', '#comment', '#comment_block'])) {
                                $body .= $line.PHP_EOL;
                            }
                        }
                        }
    ...
    public static function parseRfcAddress($address_string)
    {
        return CerberusUtils::parseRfcAddressList($address_string);
    }

    public static function convertEncoding($text, $charset = null)
    {
    ...
    public static function fixQuotePrintableArray($input, $encoding = null)
    {
    ...
    public static function fixQuotePrintableString($input, $encoding = null)
    {
        $out = '';
