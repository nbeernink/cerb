...
class CerberusMail
{
    ...
    public static function parseRfcAddresses($string, $exclude_controlled_addresses = false)
    {
        $results = [];
    ...
    private static function _parseCustomHeaders(array $headers)
    {
        if (!is_array($headers)) {
            return [];
        }
    ...
    public static function quickSend($to, $subject, $body, $from_addy = null, $from_personal = null, $custom_headers = [], $format = null, $html_template_id = null, $file_ids = [])
    {
        try {
    ...
    public static function compose($properties)
    {
        /*
         'group_id'
         'bucket_id'
        ...
        $toList = self::parseRfcAddresses($toStr);
        ...
        try {
            ...
            // Cc
            $ccList = self::parseRfcAddresses($cc);
            ...
            // Bcc
            $bccList = self::parseRfcAddresses($bcc);
        ...
        if (isset($ticket_reopen) && !empty($ticket_reopen)) {
            ...
            } elseif (false !== (@$reopen_at = strtotime($ticket_reopen))) {
                $fields[DAO_Ticket::REOPEN_AT] = $reopen_at;
            }
        }
        ...
        // Status
        if (in_array($status_id, [Model_Ticket::STATUS_WAITING, Model_Ticket::STATUS_CLOSED])) {
    ...
    public static function sendTicketMessage($properties = [])
    {
        $settings = DevblocksPlatform::getPluginSettingsService();
        ...
        */
        ...
        try {
            ...
            // Forward or overload
                ...
                // To
                $aTo = self::parseRfcAddresses($properties['to']);
                if (is_array($aTo)) {
                    foreach ($aTo as $k => $v) {
                        if (!empty($v['personal'])) {
            ...
            if (!empty($properties['cc'])) {
                $aCc = self::parseRfcAddresses($properties['cc']);
                if (is_array($aCc)) {
                    foreach ($aCc as $k => $v) {
                        if (!empty($v['personal'])) {
            ...
            if (!empty($properties['bcc'])) {
                $aBcc = self::parseRfcAddresses($properties['bcc']);
                if (is_array($aBcc)) {
                    foreach ($aBcc as $k => $v) {
                        if (!empty($v['personal'])) {
        ...
        if (isset($properties['status_id'])) {
            ...
            if (isset($properties['ticket_reopen']) && !empty($properties['ticket_reopen'])) {
                ...
                } elseif (false !== (@$reopen_at = strtotime($properties['ticket_reopen']))) {
                }
            }
    ...
    public static function relay($message_id, $emails, $include_attachments = false, $content = null, $actor_context = null, $actor_context_id = null)
    {
        $mail_service = DevblocksPlatform::getMailService();
        $settings = DevblocksPlatform::getPluginSettingsService();
    ...
    public static function reflect(CerberusParserModel $model, $to)
    {
        try {
    ...
    private static function _generateBodyMarkdown(&$mail, &$content, $group_id = 0, $bucket_id = 0, $html_template_id = 0)
    {
        $embedded_files = [];
        $exclude_files = [];
