...
class PageSection_SetupSimulator extends Extension_PageSection
{
    ...
    public function generateTicketsJsonAction()
    {
        ...
        try {
            ...
            foreach ($emails as $template) {
                ...
                CerberusParser::parseMessage($message, ['no_autoreply' => true]);
            }

            echo json_encode(['status' => true, 'message' => sprintf('Success!  %d simulated tickets were generated for %s', $how_many, htmlspecialchars($address))]);
    ...
    public function generateTasksJsonAction()
    {
        try {
            @$how_many = DevblocksPlatform::importGPC($_POST['how_many'], 'integer', 0);
            ...
            echo json_encode(['status' => true, 'message' => sprintf('Success! %d simulated tasks were generated.', $how_many)]);
    ...
    public function generateOrgsJsonAction()
    {
        try {
            @$how_many = DevblocksPlatform::importGPC($_POST['how_many'], 'integer', 0);
            ...
            echo json_encode(['status' => true, 'message' => sprintf('Success! %d simulated orgs were generated.', $how_many)]);
